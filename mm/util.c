  ...
  unsigned long flag, unsigned long pgoff)
{
	...
	if (!ret) {
retry:
		down_read(&mm->mmap_lock); // 上的是读锁
		//Use the ephemeral heap
		if(mm->eheap && (rounded_len>>PAGE_SHIFT)<=atomic64_fetch_sub(rounded_len>>PAGE_SHIFT, &mm->eheap->heap_free_pages)){ // 使用原子操作
			//is even this necessary?, currently yes for the scenarios 
			//that we change the fast_mapping_vma; (refeeding stach address space)
			ret = do_mmap_pgoff_ephemeral(file, addr, len, prot, flag, pgoff, &populate, &uf); // IMPORTANT
		...
		//expand the heap
			...
			new_vma = 1;
			ret = do_mmap(NULL, NULL, fast_mapping_vma_size, PROT_READ|PROT_WRITE, MAP_DAXVM_EPHEMERAL_HEAP, 0, &populate, &uf); // 为堆分配空间
			BUG_ON(ret<=0);
			vma = find_vma(mm,ret);
      BUG_ON(!vma);
heapsetup:
