			...
			struct rb_node *rb_parent)
{
	...
	if (vma->vm_file) {
		mapping = vma->vm_file->f_mapping;
		i_mmap_lock_write(mapping); // lock mapping->i_mmap
	}
	else BUG();
	...
	__vma_link_file(vma); // still insert into mapping->i_mmap
...
把新创建的 vma 结构连接到进程的虚拟内存区链表和红黑树中。https://zhuanlan.zhihu.com/p/546009181
*/
		...
		struct list_head *uf)
{
	...
	// 沒有vma_merge
	vma = vm_area_alloc(mm);  // 1. 申请一个虚拟内存区管理结构(vma)
	if (!vma) {
		error = -ENOMEM;
		goto unacct_error;
	}

	vma->vm_start = addr;   // 2. 设置vma结构各个字段的值
	vma->vm_end = addr + len;
	...
	vma_link_ephemeral(mm->eheap, vma, prev, rb_link, rb_parent); // 4. 把 vma 结构连接到进程虚拟内存区的链表和红黑树中。
			...
			struct list_head *uf)
{
	...
	addr = get_unmapped_area_ephemeral(file, addr, len, pgoff, flags); // 首先，调用 get_unmapped_area() 函数来获取进程没被使用的虚拟内存区，并且返回此内存区的首地址。
  ...
  vm_flags |= VM_DAXVM_EPHEMERAL;
	...
	addr = mmap_region_ephemeral(file, addr, len, vm_flags, pgoff, uf); // 然后，调用 mmap_region() 函数继续进行映射操作。
...
unsigned long get_unmapped_area_ephemeral(struct file *file, unsigned long addr, unsigned long len, unsigned long pgoff, unsigned long flags)
{
	...
	//currently no locks!
	addr = atomic64_fetch_add(len, &current->mm->eheap->heap_base); // IMPORTANT, this is weird...not looking into the heap，单调增？
		...
		struct list_head *uf, bool downgrade)
{
  ...
  len = round_up(len+(start-round_down(start, PMD_SIZE)),PMD_SIZE);
	...
	ephemeral_mm = find_ephemeral_mm(mm,start); // 找到包含对应地址的ephemeral_mm
  if(!ephemeral_mm) {
  ...
  BUG_ON(end > ephemeral_mm->vm_end);
	...
	spin_lock(&ephemeral_mm->ephemeral_lock); // 上锁
	ephemeral_vma = find_ephemeral_vma(ephemeral_mm, start); // ephemeral_mm_rb中找到包含对应地址的ephemeral_vma
	spin_unlock(&ephemeral_mm->ephemeral_lock);
  if(!ephemeral_vma) {
   ...
   while(ephemeral_vma) {
	...
	if (ephemeral_vma->vm_flags & VM_DAXVM_BATCHING) { // 批量unmap
		bool defer=0;
		if (cpumask_any_but(mm_cpumask(mm), smp_processor_id()) < nr_cpu_ids)
			defer=1;

		if(defer) {
			struct vm_area_struct *zombie;
			struct vm_area_struct *zombie_next;
	...
	spin_lock(&ephemeral_mm->ephemeral_lock); // 非批量unmap
...
#ifdef CONFIG_DAXVM
	atomic_set(&mm->daxvm_munmap_inprogress, 1); // IMPORTANT
#endif
